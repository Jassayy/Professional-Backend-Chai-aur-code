...
const refreshAccessToken = asyncHandler(async (req, res) => {
  //access the token from cookies which user is sending
  //chech if u have the same token in ur db
  //yes? then give access token to user again
...
const changeCurrentPassword = asyncHandler(async (req, res) => {
  ...
  if (newPassword !== oldPassword) {
    throw new ApiError(400, "New password must not be the same as the old one");
  }
